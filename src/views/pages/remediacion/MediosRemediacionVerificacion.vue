<template>
    <div>
        ...
        <vx-card no-shadow card-border id="div-detalle-persona-with-loading" class="mt-base vs-con-loading__container">
            ...
            <vx-card
                title="Medida de Remediación"
                title-color="success"
                :subtitle="medidaRemediacion"
                subtitle-color="primary"
                class="mb-6">

                <div class="flex flex-wrap items-center justify-start">
                    <div>
                        <small><strong class="font-bold">Deficiencia de SCI:</strong> {{ titleDeficiencia }}</small>
                        <span class="text-grey flex items-start justify-start mt-0">
                            <small><strong class="font-bold">Eje:</strong><span class="text-sm ml-2">{{ titleEje }}</span></small>
                        </span>
                        <span class="text-grey flex items-start justify-start mt-0">
                            <small><strong class="font-bold">Componente:</strong><span class="text-sm ml-2">{{ titleComponente }}</span></small>
                        </span>
                    </div>
        ...
        <vs-popup title="Responsables y Actividades/Tareas" :active.sync="popupAsignacion" id="id-popup-asignacion">
            <vx-card class="mb-4" no-shadow card-border>
                ...
                <div class="mb-4">
                    <label class="text-sm opacity-75">Responsable de Area/Gerencia/Dependencia</label>
...
export default {
    ...
    created(){
        ...
        this.getMedidaRemediacion(this.parametro);
    ...
    methods: {
        getMedidaRemediacion(value){
            this.$vs.loading();
            this.$store.dispatch('remediacion/findMedidaRemediacion', value)
            .then(response => {
                ...
                if(response.status === 200){
                    ...
                    this.medidaRemediacion = data.remediacion;
                    this.titleDeficiencia = data.deficiencia.componente.descripcion;
                    this.titleEje = data.deficiencia.componente.eje.descripcion;
                    this.titleComponente = data.deficiencia.componente.descripcion
                }
            })
            .catch(error => {
                this.$vs.loading.close();
        ...
        getMedioVerificacion(value){
            this.$vs.loading();
            this.$store.dispatch('remediacion/fetchMedioVerificacionByMedidaRemediacion', value)
            .then(response => {
                this.$vs.loading.close();
                if(response.status === 200){
        ...
        asignarResponsable(){
            ...
            this.$vs.loading();
            this.$store.dispatch('remediacion/saveOficinasMedioVerificacion', payload)
            .then(response => {
                this.$vs.loading.close();
        ...
        agregarActividad(){
            ...
            this.$vs.loading();
            this.$store.dispatch('remediacion/saveActividadMedioVerificacion', payload)
            .then(response => {
                this.$vs.loading.close();
                if(response.status === 201){
        ...
        eliminarActividad(){
            this.$vs.loading();
            this.$store.dispatch('remediacion/deleteActividadControl', this.actividadId)
            .then(response => {
                this.$vs.loading.close();
