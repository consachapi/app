<template>
    <div id="data-medio-verificacion-control-view">
        <vx-card class="mb-base" no-shadow card-border>
            ...
            <vs-table stripe ref="table" :data="itemsVerificacionControl" pagination :max-items="itemsPerPage" search>
                
                <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
                    <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
                        <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ itemsVerificacionControl.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : itemsVerificacionControl.length }} de {{ itemsVerificacionControl.length }}</span>
                ...
                <template slot="thead">
                    ...
                    <vs-th class="">MEDIDAD DE CONTROL</vs-th>
                ...
                <template slot-scope="{data}">
                    <tbody>
                        <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                            ...
                            <vs-td :data="data[indextr].medidasControl">
                                {{ data[indextr].medidasControl.mcontrol }}
                            </vs-td>
            ...
            <div class="flex flex-wrap-reverse items-center flex-grow justify-end">
                <p class="truncate text-sm">
                    Mostrando registros del <strong>{{ currentPage * itemsPerPage - (itemsPerPage - 1) }}</strong> al <strong>{{ itemsVerificacionControl.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : itemsVerificacionControl.length }}</strong> de un total de <strong>{{ itemsVerificacionControl.length }}</strong> registros
                </p>
            </div>
...
export default {
    data(){
        return {
            itemsVerificacionControl: [],
            itemsPerPage: 10,
            isMounted: false,

            itemsMedidaControl: [],
            selectedMedidaControl: null
        }
    },
    ...
    methods: {
        getMediosVerficacion(){
            this.$vs.loading();
            this.$store.dispatch('control/fecthMedioVerficacionByUsuario')
            .then(response => {
                this.$vs.loading.close();
                if(response.status === 200){
                    this.itemsVerificacionControl = response.data;
                }
            })
            .catch(error => {
                this.$vs.loading.close();
...
#data-medio-verificacion-control-view {
.vs-con-table {
